...
def get_dicts_new(path_, sent_seg, tag_scheme='BIES', crf=1):
    char2idx = {'<P>': 0, '<UNK>': 1, '<#>': 2}
    unk_chars_idx = []
    idx = 3
    language_dirs  = os.listdir(path_)
    for language_dir in language_dirs:
        if 'UD_' in language_dir:
            char2idx[language_dir[3:-1]] = idx
            idx = idx+1
    language_dirs  = os.listdir(path_)
    for language_dir in language_dirs:
        if 'UD_' in language_dir:
...
def chop_new(line, ad_s, limit, language_idx):
    out = []
    chopped = False
    while len(line) > 0:
        if chopped:
            s_line = line[:limit - 1]
            s_line = [language_idx] + [ad_s] + s_line
        else:
            chopped = True
            s_line = line[:limit]
        out.append(s_line)
...
def get_input_vec_new(path_, char2idx, tag2idx, limit=500, sent_seg=False, is_space=True, train_size=-1, ignore_space=False):
    ct = 0
    max_len = 0
    space_idx = None
    if is_space is True:
    ...
    if sent_seg:
        ...
        for language_dir in language_dirs:
            ...
            for line in codecs.open(path + '/' + fname, 'r', encoding='utf-8'):
                ...
                if len(line) == 0:
                    ...
                    x1.append(char2idx[language_dir[3:-1]])
                    if is_space == 'sea':
                        line = pre_token(line)
                    for ch in line:
                        if len(ch.strip()) == 0:
                            ...
                            x1.append(char2idx[ch])
                            x2.append(char2idx[ch])
                        else:
                            x1.append(char2idx['<UNK>'])
                            x2.append(char2idx['<UNK>'])
                    if is_space is True and not ignore_space:
                        x1 = [space_idx] + x1
                        x2 = [space_idx] + x2
                    x2.append(char2idx[language_dir[3:-1]])
                    x1_indices += x1
                    x2_indices += x2
                ...
                if 0 < train_size <= n_sent:
                    break
        x1_indices = chop(x1_indices, char2idx['<#>'], limit)
        x2_indices = chop(x2_indices, char2idx['<#>'], limit)
        y_indices = chop(y_indices, tag2idx['I'], limit)
        max_len = limit
    else:
        language_dirs = os.listdir(path_)
        ...
        if l_count > 0:
            print '%d (out of %d) sentences are chopped.' % (l_count, s_count)
    return [x1_indices],[x2_indices], [y_indices], max_len
...
def get_input_vec_raw_test_new(path, test_language, fname, char2idx, lines=None, limit=500, sent_seg=False, is_space=True, ignore_space=False):
    max_len = 0
    space_idx = None
    is_first = True
    if is_space is True:
    ...
    if sent_seg:
        for line in lines:
            ...
            if is_space == 'sea':
                ...
                line = ''.join(line.split())
            x1.append(char2idx[test_language[3:-1]])
            for ch in line:
                if len(ch.strip()) == 0:
                    x1.append(char2idx[' '])
                    x2.append(char2idx[' '])
            ...
            if is_space is True and not ignore_space:
                if is_first:
                    ...
                    x1 = [space_idx] + x1
                    x2 = [space_idx] + x2
            x2.append(char2idx[test_language[3:-1]])
            x1_indices += x1
            x2_indices += x2
            x1 = []
            x2 = []
        x1_indices = chop_new(x1_indices, char2idx['<#>'], limit)
        x2_indices = chop_new(x2_indices, char2idx['<#>'], limit)
        max_len = limit
    else:
        for line in lines:
            line = line.strip()
            if len(line) > 0:
                ...
                if len(line) > limit:
                    l_count += 1
                    chop_x1 = chop_new(x1, char2idx['<#>'], limit, char2idx[test_language[3:-1]])
                    chop_x2 = chop_new(x2, char2idx['<#>'], limit, char2idx[test_language[3:-1]])
                    x1_indices += chop_x1
                    x2_indices += chop_x2
                else:
                    x1_indices.append(x1)
        ...
        if l_count > 0:
            print '%d (out of %d) sentences are chopped.' % (l_count, s_count)
    return [x1_indices], [x2_indices], max_len


def get_input_vec_raw_new(path_, fname, char2idx, lines=None, limit=500, sent_seg=False, is_space=True, ignore_space=False):
    max_len = 0
    space_idx = None
    is_first = True
    if is_space is True:
    ...
    for language_dir in language_dirs:
        ...
        #     # else:
        #     #     real_path = path + '/' + fname
        lines = codecs.open(path + '/' + fname, 'r', encoding='utf-8')
        ...
        if sent_seg:
            for line in lines:
                ...
                x1 = []
                x2 = []
            x1_indices = chop(x_indices1, char2idx['<#>'], limit)
            x2_indices = chop(x_indices2, char2idx['<#>'], limit)
            max_len = limit
        else:
            for line in lines:
                line = line.strip()
                if len(line) > 0:
                    ...
                    if len(line) > limit:
                        l_count += 1
                        chop_x1 = chop_new(x1, char2idx['<#>'], limit, char2idx[language_dir[3:-1]])
                        chop_x2 = chop_new(x2, char2idx['<#>'], limit, char2idx[language_dir[3:-1]])
                        x1_indices += chop_x1
                        x2_indices += chop_x2
                    else:
                        x1_indices.append(x1)
...
def buckets_new(x1, x2, y, size=50):
    assert len(x1[0]) == len(y[0]) == len(x2[0])
    num_inputs = len(x1)
    #num_inputs = 1
    samples = x1 + x2 + y
    num_items = len(samples)
    ...
    for item in xy:
        ...
        for i in range(num_items):
            ...
            bucks[i][idx].append(item[i])
    # pdb.set_trace()
    return bucks[:num_inputs], bucks[num_inputs: 2*num_inputs], bucks[2*num_inputs:]
    ...
    if bucket_len_c is None:
        ...
        for i, item in enumerate(xy):
            ...
            for idx in range(num_tags + num_inputs):
                padded[idx].append(pad_zeros(item[idx], max_len-idx))
...
def pad_bucket_new(x1, x2, y, limit, bucket_len_c=None):
    assert len(x1[0]) == len(y[0])
    num_inputs1 = len(x1)
    num_inputs2 = len(x2)

    num_tags = len(y)
    padded = [[] for _ in range(num_tags + num_inputs1+num_inputs2)]
    bucket_counts = []
    samples = x1 + x2 + y
    xy = zip(*samples)
    if bucket_len_c is None:
        ...
        for i, item in enumerate(xy):
            ...
            bucket_len_c.append(max_len)
            bucket_counts.append(len(item[0]))
            for idx in range(num_inputs1 + num_inputs2):
                padded[idx].append(pad_zeros(item[idx], max_len))
            padded[2].append(pad_zeros(item[2],max_len-1))
        print 'Number of buckets: ', len(bucket_len_c)
    # else:
    #     idy = 0
    #     # pdb.set_trace()
    ...
    #             for idx in range(num_tags + num_inputs):
    #                 padded[idx].append(pad_zeros(item[idx], bucket_len_c[idy]))
    return padded[:num_inputs1], padded[num_inputs1: num_inputs1+num_inputs2], \
           padded[num_inputs1+num_inputs2:], bucket_len_c, bucket_counts
...
def decode_chars_new(idx, idx2chars):
    out = []
    for line in idx:
        line = np.trim_zeros(line)
        line = line[1:]
