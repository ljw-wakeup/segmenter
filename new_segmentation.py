...
parser.add_argument('-pn', '--new_path',default=None, help='Path of workstation for new experiments')
...
parser.add_argument('-tr',"--test_result_path", default=None, help='Test result output path')
...
if args.action == 'train':
    ...
    print 'Reading data......'
    dir_names = os.listdir(path_)
    train_file = None
    dev_file = None
    is_space=None

    for dir_l in dir_names:
        path = path_ + '/' + dir_l
        ...
        is_space = True
        if 'Chinese' in dir_l or 'Japanese' in dir_l or args.format == 'mlp2':
            is_space = False
    ...
    char2idx, unk_chars_idx, idx2char, tag2idx, idx2tag, trans_dict = toolbox.get_dicts_new(path_, args.sent_seg, args.tags,
                                                                                        args.crf)

    if args.embeddings is not None:
        print 'Reading embeddings...'
        short_emb = args.embeddings[args.embeddings.index('/') + 1: args.embeddings.index('.')]
        if args.reset or not os.path.isfile(path_ + '/' + short_emb + '_sub.txt'):
            toolbox.get_sample_embedding(path_, args.embeddings, char2idx)
        emb_dim, emb, valid_chars = toolbox.read_sample_embedding(path_, short_emb, char2idx)
    ...
    train_x1,train_x2, train_y, max_len_train = toolbox.get_input_vec_new(path_, char2idx, tag2idx,
                                                            limit=args.sent_limit, sent_seg=args.sent_seg,
                                                            is_space=is_space, train_size=args.train_size,
                                                            ignore_space=args.ignore_space)

    dev_x1, dev_x2, max_len_dev = toolbox.get_input_vec_raw_new(path_, 'raw_dev.txt', char2idx, limit=args.sent_limit,
                                                   sent_seg=args.sent_seg, is_space=is_space,
                                                   ignore_space=args.ignore_space)
    if args.sent_seg:
        print 'Joint sentence segmentation...'
    else:
        print 'Training set: %d instances; Dev set: %d instances.' % (len(train_x1[0]), len(dev_x1[0]))

    nums_grams = None
    ng_embs = None

    if args.ngram > 1 and (args.reset or  not os.path.isfile(path_ + '/' + str(args.ngram) + 'gram.txt')):
    ...
    if args.ngram > 1:
        ...
        train_gram = toolbox.get_gram_vec(path_, 'tag_train.txt', gram2idx, limit=args.sent_limit,sent_seg=args.sent_seg,
                                          is_space=is_space, ignore_space=args.ignore_space)
        dev_gram = toolbox.get_gram_vec(path_, 'raw_dev.txt', gram2idx, is_raw=True, limit=args.sent_limit,
    ...
    # for i in range(80058):
    #     assert len(train_x1[0][i]) == len(train_x2[0][i])
    ...
    # len(train_y[0])
    # 16542
    b_train_x1, b_train_x2, b_train_y = toolbox.buckets_new(train_x1, train_x2, train_y, size=args.bucket_size)
    b_train_x1, b_train_x2, b_train_y, b_lens, b_count = toolbox.pad_bucket_new(b_train_x1, b_train_x2, b_train_y, max_len)

    b_dev_x1 = [toolbox.pad_zeros(dev_x_i, max_len) for dev_x_i in dev_x1]
    b_dev_x2 = [toolbox.pad_zeros(dev_x_i, max_len) for dev_x_i in dev_x2]
    b_dev_y_gold = []

    for language_dir in dir_names:
        path = path_ + '/' + language_dir
        for line in codecs.open(path + '/tag_dev_gold.txt', 'r', encoding='utf-8'):
            line = line.strip()
            b_dev_y_gold.append(line)
    #b_dev_y_gold = [line.strip() for line in codecs.open(path_ + '/tag_dev_gold.txt', 'r', encoding='utf-8')]
    # pdb.set_trace()
    ...
    if transducer is not None:
        ...
        with transducer_graph.as_default():
            with tf.variable_scope("transducer") as scope:
                trans_model = Seq2seq(path_ + '/' + model_file + '_transducer')
    ...
    if args.ensemble:
        if not os.path.isfile(model_path + '/' + model_file + '_1_model') or not os.path.isfile(model_path + '/' + model_file +
                                                                                          '_1_weights.index'):
            raise Exception('Not any model file or weights file under the name of ' + model_file + '.')
        fin = open(model_path + '/' + model_file + '_1_model', 'rb')
    else:
        if not os.path.isfile(model_path + '/' + model_file + '_model') or not os.path.isfile(model_path + '/' + model_file +
                                                                                        '_weights.index'):
            raise Exception('No model file or weights file under the name of ' + model_file + '.')
        fin = open(model_path + '/' + model_file + '_model', 'rb')

    weight_path = model_path + '/' + model_file
    ...
    char2idx, unk_chars_idx, idx2char, tag2idx, idx2tag, trans_dict = toolbox.get_dicts_new(work_path, sent_seg, tag_scheme, crf)
    # pdb.set_trace()
    trans_char_num = len(char2idx)

    if ngram > 1:
        grams = toolbox.read_ngrams(work_path, ngram)

    new_chars, new_grams = None, None

    test_x1, test_x2, test_y, raw_x, test_y_gold = None, None, None, None, None
    ...
    if args.action == 'test':
        test_language = args.test
        # f_names = os.listdir(path)
        # if test_file is None:
        #     for f_n in f_names:
        #         if 'ud-test.conllu' in f_n:
        #             test_file = f_n
        assert test_language is not None
        test_file = None
        cat = 'other'
        # if 'Chinese' in path or 'Japanese' in path:
        #     cat = 'zh'
        f_names = os.listdir(work_path + '/' + test_language)
        for f_n in f_names:
            if 'ud-test.conllu' in f_n :
                test_file = f_n
        test_language_dir = work_path + '/' + test_language
        for line in codecs.open(test_language_dir + '/' + test_file, 'r', encoding='utf-8'):
            if len(line) < 2:
                break
            if '# sentence' in line or '# text' in line:
                cat = 'gold'
        reader.get_raw(test_language_dir, test_file, 'raw_test.txt', cat, form=args.format)

        raws_test = reader.raw(test_language_dir + '/raw_test.txt')
        test_y_gold = reader.test_gold(test_language_dir + '/' + test_file, form=args.format, is_space=is_space,
                                       ignore_mwt=args.ignore_mwt)

        new_chars = toolbox.get_new_chars(test_language_dir + '/raw_test.txt', char2idx, is_space)
        ...
        test_x1, test_x2, max_len_test = toolbox.get_input_vec_raw_test_new(test_language_dir, test_language, 'raw_test.txt', char2idx, limit=args.sent_limit + 100,
        ...
        if sent_seg:
            ...
            print 'Test set: %d instances.' % len(test_x1[0])

        # if ngram > 1:
        #     gram2idx = toolbox.get_ngram_dic(grams)
        #     new_grams = toolbox.get_new_grams(work_path + '/' + test_file, gram2idx, is_space=is_space)
        #
        #     test_grams = toolbox.get_gram_vec(work_path, 'raw_test.txt', gram2idx, is_raw=True, limit=args.sent_limit + 100,
        #                                       sent_seg=sent_seg, is_space=is_space, ignore_space=args.ignore_space)
        #     test_x += test_grams

        for k in range(len(test_x1)):
            test_x1[k] = toolbox.pad_zeros(test_x1[k], max_step)
        for k in range(len(test_x2)):
            test_x2[k] = toolbox.pad_zeros(test_x2[k], max_step)
    ...
    if args.ensemble:
        idx = 1
        main_sess = []
        while os.path.isfile(work_path + '/' + model_file + '_' + str(idx) + '_weights.index'):
    ...
    with tf.device(gpu_config):
        ...
        test_language_dir = work_path + '/' + test_language
        if args.action == 'test':
            test_y_raw = [line.strip() for line in codecs.open(test_language_dir + '/raw_test.txt', 'rb', encoding='utf-8')]
            model.test(test_x1, test_x2, test_y_raw, test_y_gold, idx2tag, idx2char, unk_chars_idx, sub_dict, trans_dict, sess,
                       transducer=trans_model, ensemble=args.ensemble, batch_size=args.test_batch, sent_seg=sent_seg,
                       bias=args.segmentation_bias, outpath=args.output_path, trans_type=args.transduction_type, test_result_path=args.test_result_path)
