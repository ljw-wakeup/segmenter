...
class BiLSTM(object):
    ...
    def __init__(self, cell_dim, nums_layers=1, p=0.5, fw_cell=None, bw_cell=None, state=False, name='biLSTM',
        ...
        self.scope = scope
        if fw_cell is None:
            self.lstm_cell_fw = tf.contrib.nn.LSTMCell(self.cell_dim, state_is_tuple=True)
        else:
            self.lstm_cell_fw = fw_cell
        if bw_cell is None:
            self.lstm_cell_bw = tf.contrib.nn.LSTMCell(self.cell_dim, state_is_tuple=True)
        else:
            self.lstm_cell_bw = bw_cell
        #assert 0. <= p < 1

    def __call__(self, input_tf,input_tb, input_ids1):
        self.input_f = input_tf
        self.input_b = input_tb
        self.input_ids = input_ids1
        #if self.p > 0.:
        ...
        int_states_f, final_states_f = tf.nn.dynamic_rnn(self.lstm_cell_fw, self.input_f,
                                                                   sequence_length=self.length, dtype=tf.float32,
                                                                   scope=self.scope + 'forward')
        int_states_b, final_states_b = tf.nn.dynamic_rnn(self.lstm_cell_bw, self.input_b,
                                                         sequence_length=self.length, dtype=tf.float32,
                                                         scope=self.scope + 'backward')
        int_states_f = int_states_f[:, 1:, :]
        int_states_b = int_states_b[:, :-1, :]
        self.output = tf.concat([int_states_f, int_states_b],2)
        if self.state:
